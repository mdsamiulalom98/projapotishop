...
class OrderController extends Controller
{
    ...
    public function index($slug, Request $request)
    {
        ...
        return view('backEnd.order.index', compact('show_data', 'order_status', 'users', 'steadfast', 'pathao_info', 'pathaostore', 'pathaocities'));
    }
    ...
    public function order_process(Request $request)
    {
        ...
        if ($request->status == 6) {
            ...
            $seller = Customer::find($order->reseller_id);
    ...
    public function order_status(Request $request)
    {
        ...
        foreach ($orders as $order) {
            ...
            if ($request->order_status == 6 && $old_status != 6) {
                $orders_details = OrderDetails::where('order_id', $order->id)->get();
                foreach ($orders_details as $order_detail) {
                    if ($order_detail->type == 1) {
                        $product = Product::select('id', 'stock')->find($order_detail->product_id);
            ...
            if ($request->order_status == 8 && $old_status != 8) {
                $total_commision = $order->commision;
                $seller = Customer::find($order->reseller_id);
                $seller->balance -= $total_commision;
                $seller->save();
    ...
    {
        OrderDetails::where('id', $request->item)->delete();
    ...
    public function order_update(Request $request)
    {
        ...
        foreach ($order->orderdetails as $orderdetail) {
            ...
            $item = $cartItems->firstWhere('id', $orderdetail->product_id);
            if (!$item) {
                $orderdetail->delete();
            }
        }
