                ...
                <form action="{{route('expense.update')}}" method="POST" class="row" data-parsley-validate="" name="editForm"  enctype="multipart/form-data">
                    ...
                    <div class="col-sm-12">
                        <div class="form-group mb-3">
                            <label for="expense_cat_id" class="form-label">Category *</label>
                             <select class="form-control select2 @error('expense_cat_id') is-invalid @enderror" id="expense_cat_id" name="expense_cat_id" value="{{ old('expense_cat_id') }}" data-placeholder="Choose ..."required>
                                <optgroup >
                                    <option value="">Choose..</option>
                                    @foreach($categories as $key=>$value)
                                    <option value="{{$value->id}}" @if($value->id == $edit_data->expense_cat_id) selected @endif>{{$value->name}}</option>
                                    @endforeach
                                </optgroup>
                            </select>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="date" class="flatdate form-control @error('date') is-invalid @enderror" name="date" value="{{ $edit_data->date }}" id="date">
                            @error('date')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
