      ...
      <div class="card">
        <div class="card-body">
          <form action="{{route('smsgeteway.update')}}" method="POST" class="row" data-parsley-validate="" enctype="multipart/form-data">
            ...
            <div class="col-sm-3 mb-3">
              <div class="form-group">
                <label for="register_verify" class="d-block">Register Verify </label>
                <label class="switch">
                  <input type="checkbox" value="1" @if($sms->register_verify==1)checked @endif name="register_verify" />
                  <span class="slider round"></span>
                </label>
                @error('register_verify')
                <span class="invalid-feedback" role="alert">
                  <strong>{{ $message }}</strong>
                </span>
                @enderror
